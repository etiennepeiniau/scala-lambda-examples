...
import commons.ApiGatewayInternals.CORS_HEADERS
  ...
  def recordRequest(request: AwsProxyRequest): AwsProxyResponse = {
    request.pathParameters.flatMap(_.get("code")).map(_.toInt).map(code => {
      ...
      if (item == null) {
        ...
        AwsProxyResponse(200, CORS_HEADERS, newItem.toJSON)
      }
      else {
        ...
        AwsProxyResponse(200, CORS_HEADERS, updatedItem.toJSON)
      }
    }).getOrElse(parameterError)
  }

  def parsingError(error: Error): AwsProxyResponse = {
    AwsProxyResponse(500, CORS_HEADERS, error.asJson.noSpaces)
  }

  def parameterError: AwsProxyResponse = {
    AwsProxyResponse(400, CORS_HEADERS, ErrorResponse("Code path parameter is missing").asJson.noSpaces)
  }
